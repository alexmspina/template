...
func Build() error {
	...
	cmd := exec.Command("go", "build", "./cmd/consumer/consumer.go")
...
func BuildMacOS() error {
	...
	// Build executable from cmd/portapp.go
	fmt.Println("Building...")
	cmd := exec.Command("env", "GOOS=darwin", "GOARCH=amd64", "go", "build", "./cmd/consumer/consumer.go")
...
func Install() error {
	...
	return os.Rename("./consumer", "/usr/bin/consumer")
}
